...
function Connection(config) {
  ...
  this._maxTasks = config.agent === undefined ? 5 : config.agent.maxSockets;
}
...
extend(Connection.prototype, {
  ...
  addQueue: function addQueue(action) {
    ...
    if (this._currentTasks < this._maxTasks) this.drainQueue();
  },
  drainQueue: function drainQueue() {
    while (this._queue.length > 0 && this._currentTasks < this._maxTasks) {
      ...
      if (action.method === 1) {
        ...
        action.action.post(action.path, action.data, action.collection, (function (err, res) {
          self._currentTasks--;
          this.callback(err, res);
          self.drainQueue();
        }).bind(action));
